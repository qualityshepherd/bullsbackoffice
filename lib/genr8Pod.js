import { addLeadingZerosToDateString, removeFuturePosts } from '../src/state.js'
...
const pod = {
  ...
  image: `${config.url}/assets/images/bbo_og.jpg`,
  author: 'brine',
  explicit: 'yes',
...
<channel>
  ...
  <itunes:keywords>Chicago Bulls, Bulls, NBA, basketball, back office, bullsbackoffice</itunes:keywords>
  <itunes:owner>
  ...
  <atom:link href="${pod.podRss}" rel="self" type="application/rss+xml" />`
    ...
    for (const podcast of podcasts) {
      ...
      // Full HTML content for podcast apps
      const safeHtml = podcast.html
        .replace(/<input[^>]*>/g, '✓') // replace checkbox with a symbol
        .replace(/src="(?!https?:\/\/)/g, `src="${config.url}/`)
        .replace(/]]>/g, ']]&gt;') // escape CDATA close

      const formattedDate = addLeadingZerosToDateString(podcast.meta.date)
  ...
  <item>
    ...
    <pubDate>${new Date(`${formattedDate}T00:04:20Z`).toUTCString()}</pubDate>
    ...
    console.log('✅ Podcast RSS generated successfully')
  } catch (err) {
...
function getImage (podobj) {
  return podobj.meta.image
    ? podobj.meta.image
    : `${config.url}/assets/images/bbo_og.jpg`
}
